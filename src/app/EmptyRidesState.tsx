...
export function EmptyRidesState({ title = "No rides yet", subtitle }: EmptyRidesStateProps) {
  // Rider mode has no "create" affordance anywhere — see store/userModeStore.ts.
  const isOrganizer = useIsOrganizer();
  const text =
    subtitle ??
    (isOrganizer
      ? "Create a ride or race, or join one with a code."
      : "Join a ride with a code, or find one to join below.");
