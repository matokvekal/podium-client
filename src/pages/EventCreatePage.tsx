...
export function EventCreatePage() {
  ...
  async function pickEventToCopy(event: EventSummary) {
    ...
    // a track they did not choose, and the copy would have been counted against it too.
    setFromRouteId(null);
  ...
  /**
   * Stores geometry as a NEW route row — POST /events/:id/route with { points }.
   *
   * Extracted unchanged from saveExtras so the uploaded-file path and the copy-from-a-ride
   * fallback send byte-identical bodies. This is the only branch that still forks a row, and
   * for an uploaded file that is correct: there is no existing track to attach.
   */
  async function postRoutePoints(
    id: string,
    route: EventRoute,
  ...
  async function saveExtras(id: string) {
    ...
    if (existingRouteAttached) {
      ...
      // that ride's own route row (copyTrackFromEvent).
      //
      // This used to fall into the `points` branch below: we re-POSTed the geometry we had just
      // fetched from the source ride, which made the server store a SECOND routes row. The two
      // rides then shared nothing — a fix to the original never reached the copy, and the
      // original track had no way to know it had been used, so it could never be counted.
      // Sending the id instead makes it one shared row, which is what
      // ELNINO_AGENT_SOURCE_OF_TRUTH_SHORT.md always said a copy was.
      //
      ...
      try {
        ...
        // way — the ride still saves with its track, it just forks the geometry exactly as it
        // did before. This is only about deploy order: it lets the client ship ahead of the
        // server without anyone losing a ride, and it stops being reached once both are up.
        await postRoutePoints(id, copiedRoute, typedDistance, typedClimb);
      }
    } else if (copiedRoute) {
      // An uploaded or drawn line — there is no existing row to attach, so this genuinely does
      // store new geometry. Instant, same-device feedback first (eventRouteStore.ts), then the
      // server so every viewer sees the same route. Awaited so navigation never races ahead of
      // route persistence.
      setEventRoute(id, copiedRoute);
      await postRoutePoints(id, copiedRoute, typedDistance, typedClimb);
    }
